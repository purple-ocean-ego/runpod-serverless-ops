...
#### シード設定 (`seed`)
- `"random"`: 各リクエストごとに異なるシード値を生成し、ファイル名に付与します（競合回避のため、`_seed[シード値]_` がファイル名に追加されます）。
- `"fixed"`: 元のJSONに記載されたシード値を維持します（ファイル名のprefixも同じ `_seed[シード値]_` になることで、キャッシュが働き、同じ内容の異なるファイルが大量生成されないようにします）。
